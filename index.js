...
function displayCart() {
  ...
  cartList.innerHTML = ""
  cart.forEach((item) => {
    let totalPrice = (item.quantity * item.price).toLocaleString("it-IT", {
      style: "currency",
      currency: "EUR",
    })
    ...
    li.innerHTML = `
            <img src="${
              item.img
            }" class="img-fluid" style="width: 100px; height: auto; margin-right: 10px;" alt="Copertina di ${
      item.title
    }">
            <div class="flex-grow-1">
                <strong>${item.title}</strong> - Quantità: ${
      item.quantity
    } x ${item.price.toFixed(2)}€ = Totale: ${totalPrice}
            </div>
...
function displayError() {
  const bookList = document.getElementById("book-list")
  bookList.innerHTML =
    '<div class="alert alert-danger" role="alert">Impossibile caricare i libri. Si prega di riprovare più tardi.</div>'
}
